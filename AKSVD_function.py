"""
 The following code segment is based on the implementation by Meyer et al. [Deep K-SVD Denoising],
 with minor modifications to suit our experiments. Original code: https://github.com/meyerscetbon/Deep-K-SVD
"""
import os
...
class SubImagesDataset(Dataset):   #用子图来训练
    def __init__(self, root_dir, image_names, sub_image_size, sigma, transform=None):
        ...
        self.number_sub_images = int(
            (w - sub_image_size + 1) * (h - sub_image_size + 1)  #number of sub_image
        ) 

        self.number_images = len(self.image_names)

    @staticmethod
    def extract_sub_image_from_image(image, sub_image_size, idx_sub_image):   #extract the sub_image of size 128*128
        w, h = np.shape(image)
    ...
    def __getitem__(self, idx):
        ...
        np.random.seed(idx)
        noise = np.random.randn(self.sub_image_size, self.sub_image_size)  #Noise with a normal distribution
...
class FullImagesDataset(Dataset):   #Get the full image and use it for testing
...
class DenoisingNet_MLP(torch.nn.Module):
    def __init__(
        ...
        self.Dict = torch.nn.Parameter(Dict_init)      #self.Dict, self.c adn self.w are trainable
        ...
        self.attention4 = CBAM(D_out_lam, 16)  
    ...
    def forward(self, x):  
        ...
        unfold = self.unfold(x)   #[1, 64, 14641] 
        ...
        #Add attention to lam to find similarities and structures.
        l = lam / self.c   #c_init = (linalg.norm(Dict_init.cpu(), ord=2)) ** 2  
        y = torch.matmul(unfold, self.Dict)  #dict：[64, 256]  y:[1, 14641, 256] 
