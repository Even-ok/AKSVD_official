"""
Mostly similar to "AKSVD_function" ,
but can be used for process visualization
"""
...
class DenoisingNet_MLP(torch.nn.Module):
    ...
    def forward(self, x):  
        ...
        '''
        ################################################## Draw the mesh before attention ################################################################
        '''
        lam = self.change_shape(lam,trans_w,trans_h)  
        fig = plt.figure()
        ...
        X, Y = np.meshgrid(X, Y)   
        Z = lam[0,0,:,:].cpu().detach().numpy()
        ax.plot_surface(X, Y, np.transpose(Z), rstride=1, cstride=1, cmap=plt.get_cmap('rainbow'))
        ax.contourf(X,Y, np.transpose(Z),zdir='z',offset=-0.15)
        ax.set_zlim(-0.15,0.05)
        plt.show()

        '''
        ################################################## Draw the mesh after attention ################################################################
        '''
        lam = self.attention4(lam)  #[1,256,121,121]
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        # X, Y value
        X = np.arange(0, trans_w, 1)
